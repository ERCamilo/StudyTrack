(function (global) {
  ...
  function renderEnrolledScheduleHTML(enrolledSubjects, scheduleData, { escapeHtml = String, escapeJsString = String } = {}) {
    return enrolledSubjects.map((subject, index) => {
      ...
      return `<div class="flex flex-col sm:flex-row sm:items-center gap-3 p-3 bg-slate-50 dark:bg-slate-700/50 rounded-xl border border-slate-200 dark:border-slate-700"><div class="flex items-center gap-3 flex-1 min-w-0"><div class="w-8 h-8 ${getSubjectColor(subject.id, index)} rounded-lg flex items-center justify-center text-white text-xs font-bold shrink-0">${safeSubjectCodePrefix}</div><div class="flex-1 min-w-0"><h4 class="font-bold text-slate-900 dark:text-white text-sm truncate">${safeSubjectName}</h4><div class="flex flex-wrap items-center gap-2 mt-1">${blocksHtml || '<span class="text-[10px] text-amber-600 dark:text-amber-400 italic">Sin horario asignado</span>'}</div></div></div><button onclick="openScheduleModal('${subjectIdJs}', '${escapeJsString(subject.name)}', null)" class="text-xs bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 px-3 py-1.5 rounded-lg font-bold hover:bg-slate-50 dark:hover:bg-slate-700 transition-all shrink-0 shadow-sm"><i class="fas fa-plus mr-1"></i>Agregar Bloque</button></div>`;
    }).join('');
  }
